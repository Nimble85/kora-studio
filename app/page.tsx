...
const seasonalReminder = (() => {
  const holidays = [
    { month: 2, day: 14, icon: '💌', title: 'День закоханих' },
    { month: 3, day: 8, icon: '🌷', title: '8 березня' },
    { month: 8, day: 24, icon: '🇺🇦', title: 'День Незалежності' },
    { month: 10, day: 31, icon: '🎃', title: 'Хелловін' },
    { month: 12, day: 25, icon: '🎄', title: 'Різдво' },
  ];
  const current = new Date(`${today}T12:00:00Z`);
  const year = current.getUTCFullYear();
  const next = holidays.flatMap((holiday) => [year, year + 1].map((value) => ({ ...holiday, date: new Date(Date.UTC(value, holiday.month - 1, holiday.day, 12)) }))).filter((holiday) => holiday.date >= current).sort((a, b) => a.date.getTime() - b.date.getTime())[0];
  const days = Math.ceil((next.date.getTime() - current.getTime()) / 86400000);
  return { icon: next.icon, title: next.title, note: `Через ${days} дн. — час підготуватися.` };
})();
...
const categoryLabels: Record<ProductCategory, string> = {
  wood: 'Свічки в дереві', plaster: 'Свічки в гіпсі', coconut: 'Свічки в кокосі', glass: 'БУРШТИН у склі', stone: 'Свічки в камені', molded: 'Формові свічки', box: 'Бокси', workshop: 'Майстер-класи', education: 'Навчання',
};
...
export default function HomePage() {
  ...
  const [form, setForm] = useState({ productId: 'wood-bark-medium', unitPrice: '1400', amount: '1400', quantity: '1', soldAt: today, channel: 'instagram' as SalesChannel, payment: 'cashless' as PaymentMethod, note: '', workshopAt: '', deposit: '0', candleChoices: [{ name: '', quantity: '1' }], instagramUrl: '' });
  ...
  const lowStockMaterials = materials.filter((material) => material.minStock > 0 && material.stock <= material.minStock);
  const workshopBookings = sales.filter((sale) => sale.workshopAt && (products.find((product) => product.id === sale.productId)?.category === 'workshop' || sale.productName.startsWith('МК ')));
  const upcomingWorkshops = [...new Set(workshopBookings.map((sale) => `${sale.workshopAt}|${sale.productName}`))].map((key) => { const [date, name] = key.split('|'); const daysUntil = Math.round((Date.parse(`${date}T12:00:00Z`) - Date.parse(`${today}T12:00:00Z`)) / 86400000); return { date, name, daysUntil, bookings: workshopBookings.filter((sale) => sale.workshopAt === date && sale.productName === name).length }; }).filter((item) => item.daysUntil >= 0 && item.daysUntil <= 3).sort((a, b) => a.daysUntil - b.daysUntil || a.name.localeCompare(b.name, 'uk'));
  const calendarBookings = workshopBookings.filter((sale) => sale.workshopAt?.startsWith(calendarMonth) && (!calendarDay || sale.workshopAt === calendarDay)).sort((a, b) => (a.workshopAt ?? '').localeCompare(b.workshopAt ?? ''));
  const calendarDayCount = new Date(Number(calendarMonth.slice(0, 4)), Number(calendarMonth.slice(5, 7)), 0).getDate();
  const calendarOffset = (new Date(`${calendarMonth}-01T12:00:00`).getDay() + 6) % 7;
  const salesYears = [...new Set(sales.map((sale) => sale.soldAt.slice(0, 4)))].sort().reverse();
  ...
  function openNewSale() {
    setEditingSale(null);
    setForm((current) => ({ ...current, soldAt: today, quantity: '1', amount: current.unitPrice, note: '', workshopAt: '', deposit: '0', candleChoices: [{ name: '', quantity: '1' }], instagramUrl: '' }));
    setShowAdd(true);
  }
  ...
  function openSaleEditor(sale: Sale) {
    setEditingSale(sale);
    setForm({ productId: sale.productId, unitPrice: String(sale.amount / Math.max(sale.quantity, 1)), amount: String(sale.amount), quantity: String(sale.quantity), soldAt: sale.soldAt.slice(0, 10), channel: sale.channel, payment: sale.payment, note: sale.note ?? '', workshopAt: sale.workshopAt ?? '', deposit: String(sale.deposit ?? 0), candleChoices: sale.candleChoices?.map((choice) => ({ name: choice.name, quantity: String(choice.quantity) })) ?? [{ name: '', quantity: '1' }], instagramUrl: sale.instagramUrl ?? '' });
    setShowAdd(true);
  }
  ...
  async function addSale() {
    ...
    const isWorkshop = product.category === 'workshop';
    const deposit = Number(form.deposit);
    const candleChoices = form.candleChoices.filter((choice) => choice.name.trim()).map((choice) => ({ name: choice.name.trim(), quantity: Number(choice.quantity) }));
    if (isWorkshop && (form.instagramUrl && !/^https:\/\/(www\.)?instagram\.com\//i.test(form.instagramUrl.trim()))) { setSyncError('Додайте коректне посилання на Instagram.'); return; }
    if (isWorkshop && (!form.workshopAt || !Number.isFinite(deposit) || deposit < 0 || deposit > amount || !candleChoices.length || candleChoices.some((choice) => !Number.isFinite(choice.quantity) || choice.quantity <= 0))) { setSyncError('Вкажіть дату МК, свічки та передплату від 0 до повної ціни.'); return; }
    setSaving(true);
    setSyncError('');
    try {
      const payload = { soldAt: new Date(`${form.soldAt}T12:00:00`).toISOString(), productId: product.id, productName: product.name, amount, quantity, channel: form.channel, payment: form.payment, note: form.note.trim() || undefined, workshopAt: isWorkshop ? form.workshopAt : undefined, deposit: isWorkshop ? deposit : undefined, candleChoices: isWorkshop ? candleChoices : undefined, instagramUrl: isWorkshop ? form.instagramUrl.trim() || undefined : undefined };
    ...
    <main className="app-shell" data-theme={dark ? 'dark' : 'light'}>
      ...
      <section className="content" onTouchStart={(event) => setTouchStartX(event.touches[0]?.clientX ?? null)} onTouchEnd={(event) => handleTabSwipeEnd(event.changedTouches[0]?.clientX ?? 0)}>
        ...
        {tab === 'home' && <>
          ...
          {upcomingWorkshops.length > 0 && <div className="section"><div className="section-title" style={{ marginBottom: 10 }}>Майстер-класи незабаром</div><div className="workshop-reminders">{upcomingWorkshops.map((item) => <button className="workshop-reminder" key={`${item.date}|${item.name}`} onClick={() => { setCalendarMonth(item.date.slice(0, 7)); setCalendarDay(item.date); setCalendarOpen(true); setTab('more'); }}><span className="workshop-reminder-copy"><strong>{item.daysUntil === 0 ? 'Сьогодні творимо магію!' : item.daysUntil === 1 ? 'Завтра зустрічаємося на МК!' : `За ${item.daysUntil} дні — час готуватися`}</strong><span>{item.name} · {shortDate(item.date)} · бронювань: {item.bookings}</span></span><ChevronRight size={17} aria-hidden="true" /></button>)}</div></div>}
          <div className="section"><div className="section-title" style={{ marginBottom: 10 }}>Підготовка до сезону</div><div className="season-card"><div className="insight"><span className="season-icon">{seasonalReminder.icon}</span><div><b>{seasonalReminder.title}</b><span>{seasonalReminder.note}</span></div></div></div></div><div className="section"><div className="section-title" style={{ marginBottom: 10 }}>Поповнити склад</div><div className="card list">{lowStockMaterials.length ? lowStockMaterials.map((material) => <div className="row" key={material.id}><b>{material.name}</b><span className="warning">{material.stock} / {material.minStock} {material.unit}</span></div>) : <span className="eyebrow">Матеріалів нижче встановленого мінімуму немає. Задайте мінімум у Складі, щоб бачити нагадування.</span>}</div></div>
        </>}
        ...
        {tab === 'sales' && <>
          ...
          <div className="section sales-section"><div className="section-head"><div><div className="section-title">Усі продажі</div><div className="eyebrow">{sales.length} записів</div></div><button className="text-action" onClick={() => setSalesListOpen((value) => !value)}>{salesListOpen ? 'Згорнути' : 'Розгорнути'}</button></div>{salesListOpen && <div className="list">{sales.length ? salesYears.map((year) => <details className="sales-period" key={year}><summary>{year} рік · {sales.filter((sale) => sale.soldAt.startsWith(year)).length} продажів</summary>{[...new Set(sales.filter((sale) => sale.soldAt.startsWith(year)).map((sale) => sale.soldAt.slice(0, 7)))].sort().reverse().map((month) => <details className="sales-period" key={month}><summary>{new Intl.DateTimeFormat('uk-UA', { month: 'long', year: 'numeric' }).format(new Date(`${month}-01T12:00:00`))} · {sales.filter((sale) => sale.soldAt.startsWith(month)).length}</summary><div className="list">{sales.filter((sale) => sale.soldAt.startsWith(month)).sort((a, b) => b.soldAt.localeCompare(a.soldAt)).map((sale) => <div className="card sale-card" key={sale.id}><div className="row"><div><b>{sale.productName}</b><div className="eyebrow">{shortDate(sale.soldAt)} · {paymentLabels[sale.payment]}</div></div><b>{money(sale.amount)}</b></div><div className="sale-meta"><span className="pill">{channelLabels[sale.channel]}</span><span className="eyebrow">× {sale.quantity}</span><div className="row-actions"><button onClick={() => openSaleEditor(sale)} aria-label="Редагувати продаж"><Pencil size={14}/></button><button onClick={() => deleteSale(sale)} aria-label="Видалити продаж"><Trash2 size={14}/></button></div></div>{sale.workshopAt && <div className="sale-note">МК {shortDate(sale.workshopAt)} · передплата {money(sale.deposit ?? 0)} · <span className={(sale.amount - (sale.deposit ?? 0)) > 0 ? 'balance-due' : 'balance-paid'}>Залишок {money(Math.max(0, sale.amount - (sale.deposit ?? 0)))}</span></div>}{sale.note && <div className="sale-note">{sale.note}</div>}</div>)}</div></details>)}</details>) : <div className="card"><span className="eyebrow">Ще немає продажів</span></div>}</div>}</div>
        </>}
        ...
        {tab === 'more' && <>
          ...
          <div className="section directory-section"><div className="section-head"><div><div className="section-title">Календар майстер-класів</div><div className="eyebrow">{workshopBookings.length} бронювань</div></div><button className="text-action" type="button" onClick={() => setCalendarOpen((value) => !value)} aria-expanded={calendarOpen} aria-controls="workshop-calendar-content">{calendarOpen ? 'Згорнути' : 'Розгорнути'}</button></div>{calendarOpen && <div id="workshop-calendar-content"><input aria-label="Місяць календаря" type="month" value={calendarMonth} onChange={(event) => { setCalendarMonth(event.target.value); setCalendarDay(''); }} /><div className="calendar-grid">{['Пн','Вт','Ср','Чт','Пт','Сб','Нд'].map((day) => <b key={day}>{day}</b>)}{Array.from({ length: calendarOffset }, (_, index) => <span key={`blank-${index}`} />)}{Array.from({ length: calendarDayCount }, (_, index) => { const date = `${calendarMonth}-${String(index + 1).padStart(2, '0')}`; const count = workshopBookings.filter((sale) => sale.workshopAt === date).length; return <button key={date} className={(calendarDay === date ? 'selected ' : '') + (count ? 'has-bookings' : '')} onClick={() => setCalendarDay((current) => current === date ? '' : date)} aria-label={`${date}, ${count} бронювань`}>{index + 1}{count > 0 && <small>{count}</small>}</button>; })}</div>{calendarDay && <button className="text-action" onClick={() => setCalendarDay('')}>Показати весь місяць</button>}<div className="list">{calendarBookings.length ? [...new Set(calendarBookings.map((sale) => sale.workshopAt))].map((date) => <div className="card" key={date}><b>{shortDate(date!)}</b>{[...new Set(calendarBookings.filter((sale) => sale.workshopAt === date).map((sale) => sale.productName))].map((name) => <div className="calendar-workshop" key={name}><div className="row"><b>{name}</b><span className="pill">{calendarBookings.filter((sale) => sale.workshopAt === date && sale.productName === name).length} бронювань</span></div>{calendarBookings.filter((sale) => sale.workshopAt === date && sale.productName === name).map((sale) => <div className="calendar-booking" key={sale.id}><span>{sale.instagramUrl ? <a href={sale.instagramUrl} target="_blank" rel="noopener noreferrer">Instagram ↗</a> : sale.productName} · {sale.candleChoices?.map((choice) => `${choice.name} × ${choice.quantity}`).join(', ')}</span><span className={(sale.amount - (sale.deposit ?? 0)) > 0 ? 'balance-due' : 'balance-paid'}>Залишок {money(Math.max(0, sale.amount - (sale.deposit ?? 0)))}</span><button onClick={() => openSaleEditor(sale)} aria-label="Редагувати бронювання"><Pencil size={14}/></button></div>)}</div>)}</div>) : <div className="card eyebrow">На цей місяць майстер-класів немає.</div>}</div></div>}</div>
      ...
      {showAdd && <div className="modal-backdrop" onClick={() => setShowAdd(false)}><div className="modal" onClick={(event) => event.stopPropagation()}><div className="section-head"><div><div className="eyebrow">Швидкий запис</div><div className="section-title">{editingSale ? 'Редагувати продаж' : '＋ Новий продаж'}</div></div><button className="icon-btn" onClick={() => setShowAdd(false)} aria-label="Закрити"><X size={18} /></button></div><div className="form">
        ...
        {products.find((product) => product.id === form.productId)?.category === 'workshop' && <><div className="field"><label>Дата проведення майстер-класу</label><input type="date" value={form.workshopAt} onChange={(event) => setForm((value) => ({ ...value, workshopAt: event.target.value }))} /></div><div className="field"><label>Передплата, ₴</label><input type="number" min="0" max={form.amount} value={form.deposit} onChange={(event) => setForm((value) => ({ ...value, deposit: event.target.value }))} /></div><div className={'workshop-balance ' + (Number(form.amount) - Number(form.deposit) > 0 ? 'balance-due' : 'balance-paid')}>Залишок до сплати: {money(Math.max(0, Number(form.amount) - Number(form.deposit)))}</div><div className="field"><label>Які свічки обрав клієнт</label>{form.candleChoices.map((choice, index) => <div className="candle-choice" key={index}><input aria-label="Назва свічки" value={choice.name} onChange={(event) => setForm((value) => ({ ...value, candleChoices: value.candleChoices.map((item, i) => i === index ? { ...item, name: event.target.value } : item) }))} placeholder="Назва свічки" /><input aria-label="Кількість свічок" type="number" min="1" value={choice.quantity} onChange={(event) => setForm((value) => ({ ...value, candleChoices: value.candleChoices.map((item, i) => i === index ? { ...item, quantity: event.target.value } : item) }))} />{form.candleChoices.length > 1 && <button className="icon-btn" onClick={() => setForm((value) => ({ ...value, candleChoices: value.candleChoices.filter((_, i) => i !== index) }))} aria-label="Прибрати свічку"><X size={15}/></button>}</div>)}<button className="text-action" onClick={() => setForm((value) => ({ ...value, candleChoices: [...value.candleChoices, { name: '', quantity: '1' }] }))}>＋ Додати свічку</button></div><div className="field"><label>Посилання на Instagram клієнта</label><input type="url" value={form.instagramUrl} onChange={(event) => setForm((value) => ({ ...value, instagramUrl: event.target.value }))} placeholder="https://instagram.com/…" /></div></>}
